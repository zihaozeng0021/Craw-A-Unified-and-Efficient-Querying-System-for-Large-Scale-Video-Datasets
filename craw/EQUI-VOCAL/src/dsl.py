...
class Color(Predicate):
    has_theta = False  # 因为Color的参数是具体颜色值，不是数值theta

    def __init__(self, parameter=None, with_hole=False):
        self.parameter = parameter  # 存储颜色值，如'red'
        self.with_hole = with_hole
        super().__init__("Color")

    def execute(self, input, label, memoize, new_memoize):
        subquery_str = utils.print_program(self)
        if subquery_str in memoize or subquery_str in new_memoize:
            return (memoize.get(subquery_str) or new_memoize[subquery_str]), new_memoize
        
        # 这里需要根据实际数据格式实现Color谓词的逻辑
        # 示例：假设input中包含物体颜色信息，判断o0的颜色是否为self.parameter
        result = np.full((len(input[0]) + 1, len(input[0]) + 1), -np.inf)
        # 实际逻辑需替换为你的数据处理代码
        indices = np.where(input[0, :, 4] == self.parameter)[0]  # 假设第4列是颜色
