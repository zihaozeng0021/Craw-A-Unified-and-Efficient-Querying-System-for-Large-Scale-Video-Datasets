...
def generate_queries(n_queries, ratio_lower_bound, ratio_upper_bound, npred, depth, max_duration, nvars, predicate_list, attr_predicate_list, max_workers, dataset_name, nattr_pred, port):
    ...
    with open("/home/sorgenfrei/EQUI-VOCAL/inputs/{}/queries.csv".format(dataset_name), "w") as csvfile:
...
def prepare_data_given_target_query(program_str, ratio_lower_bound, ratio_upper_bound, dataset_name, inputs_table_name, port=5432, sampling_rate=None):
    ...
    dsn = "dbname=myinner_db user=sorgenfrei host=localhost port={}".format(port)
    ...
    if not os.path.exists("/home/sorgenfrei/EQUI-VOCAL/inputs/{}".format(dataset_name)):
        os.makedirs("/home/sorgenfrei/EQUI-VOCAL/inputs/{}".format(dataset_name), exist_ok=True)
    with open("/home/sorgenfrei/EQUI-VOCAL/inputs/{}/{}_labels.json".format(dataset_name, program_str), 'w') as f:
...
def prepare_noisy_data(fn_error_rate, fp_error_rate, dataset_name):
    source_folder_name = os.path.join("/home/sorgenfrei/EQUI-VOCAL/inputs", dataset_name)
    target_folder_name = "/home/sorgenfrei/EQUI-VOCAL/inputs/{}-fn_error_rate_{}-fp_error_rate_{}".format(dataset_name, fn_error_rate, fp_error_rate)
...
def prepare_data_postgres(port):
    ...
    construct_train_test("/home/sorgenfrei/EQUI-VOCAL/inputs/synthetic_scene_graph_without_duration-npred_5-nattr_pred_2-40", n_train=500)
